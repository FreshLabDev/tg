...
func richMarkdown(body string) map[string]any {
	return map[string]any{
		"markdown":              body,
		"skip_entity_detection": true,
	}
...
func (c *Client) SendRichMarkdown(ctx context.Context, chatID int64, markdown string, markup *InlineKeyboardMarkup) (Message, error) {
	req := map[string]any{
		"chat_id":      chatID,
		"rich_message": richMarkdown(markdown),
	}
	if markup != nil {
		req["reply_markup"] = markup
	}
